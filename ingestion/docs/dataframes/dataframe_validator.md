...
- **test** (<code>[BaseTest](#metadata.sdk.data_quality.tests.base_tests.BaseTest)</code>) – Test definition (e.g., ColumnValuesToBeNotNull)
...
- \***tests** (<code>[BaseTest](#metadata.sdk.data_quality.tests.base_tests.BaseTest)</code>) – Variable number of test definitions
...
- **data** (<code>[Iterable](#typing.Iterable)\[[DataFrame](#pandas.DataFrame)\]</code>) – An iterable of pandas DataFrames
- **on_success** (<code>[ValidatorCallback](#metadata.sdk.data_quality.dataframes.dataframe_validator.ValidatorCallback)</code>) – Callback to execute after successful validation
- **on_failure** (<code>[ValidatorCallback](#metadata.sdk.data_quality.dataframes.dataframe_validator.ValidatorCallback)</code>) – Callback to execute after failed validation
- **mode** (<code>[FailureMode](#metadata.sdk.data_quality.dataframes.validation_results.FailureMode)</code>) – Validation mode (`FailureMode.ShortCircuit` stops on first failure)

**Returns:**

- <code>[ValidationResult](#metadata.sdk.data_quality.dataframes.validation_results.ValidationResult)</code> – Merged ValidationResult aggregating all batch validations
